...
int main(int argc, char** argv)
{
  ...
  */
	...
	if(strncmp(buf,"TX", 2) == 0){
		...
		//Esrita da leitura para um ficheiro chamado leitura.gif
		fd_leitura = open("leitura.txt", O_CREAT | O_RDWR);
		if (fd_leitura == -1) {
			perror("open");
			exit(-1);
		}

		if (write(fd_leitura,teste, strlen(teste))<0){
			perror("write fd_leitura");
			exit(-1);
		}
		////////////////////////////////////////////////////////
	...
	if (MODE == RX) {
		...
		if(strncmp(frame_read, DISC, 5) != 0) {
			...
			//SE FOR O UA FAZ CLOSE
				...
				fprintf(stderr, "UA message received correctly; Disconnection established\n");
				return 0;
			}
		...
		//VER SE RECEBEU DISC DO RX
		if(strncmp(frame_read, DISC, 5) != 0) {  //recebeu mensagem diferente de DISC, sai da função
			fprintf(stderr, "Unexpected message received; Aborting connection...\n");
			return -1;
		} else {
			fprintf(stderr, "Disconnect request accepted.\n");

			//ESCREVE UA PARA O RX
			res = write(port, UA, 5);
			fprintf(stderr, "\nUA frame sent: |%x|%x|%x|%x|%x|;\t bytes sent: %d\n\n", UA[0], UA[1], UA[2], UA[3], UA[4], res);
		}
